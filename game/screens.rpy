...
screen navigation():
    ...
    vbox:
        ...
        textbutton _("シーンを振り返る") action ShowMenu("replay_gallery")
    ...
    add Sakura("gui/sakura.png")
    add Sakura("gui/sakura2.png")
    ...
    use game_menu(_("ヒストリー"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0):
        ...
        for h in _history_list:
            ...
            window:
                ...
                ##バックログジャンプの機能追加箇所
                textbutton "ジャンプする":
                    action Confirm(_('このテキストまで戻りますか？'), yes=RollbackToIdentifier(h.rollback_identifier), no=None, confirm_selected=False)
...
init python:
    # 曲名を格納する変数
    song_name = ""
    ##song_name_duration = 0.0

screen song_name_display():
    zorder 100
    text "曲名:[song_name]" xalign 0.05 yalign 0.05 color "#fff" outlines [(2, "#000", 0, 0)] size 30
    timer 3.0 action Hide("song_name_display")